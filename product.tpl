		...
		<div id="thumbs_list">
			...
			<ul id="thumbs_list_frame">
				...
				{if isset($images)}
					...
					<li id="thumbnail_{$image.id_image}" {if $smarty.foreach.thumbnails.last} class="last_thumb"{/if}>
                ...
                {if $product->show_price AND !isset($restricted_country_mode) AND !$PS_CATALOG_MODE}
    				...
    				<div class="row_1 bordercolor">
						...
						{if $priceDisplay >= 0 && $priceDisplay <= 2}
							...
							<span id="our_price_display" class="our_price_display price">{convertPrice price=$productPrice}</span>
...
<!-- quantity wanted -->
                    ...
                    <div class="row_2 bordercolor{if ($product->on_sale) || ($product->specificPrice AND $product->specificPrice.reduction AND $productPriceWithoutReduction > $productPrice) || ($product->online_only) || ($product->specificPrice AND $product->specificPrice.reduction_type == 'percentage') || ($product->specificPrice AND $product->specificPrice.reduction_type == 'amount' && $product->specificPrice.reduction|intval !=0) || ($product->specificPrice AND $product->specificPrice.reduction)} bordered{/if}"> 

<!-- discount -->

                    	{if $product->on_sale}

                        	<span class="discount on_sale">{l s='On sale!'}</span>
...
<!-- old price -->
						...
						<div class="other-prices">
							 ...
							 {if $priceDisplay == 2}
								...
								<p id="pretaxe_price"><span id="pretaxe_price_display">{convertPrice price=$product->getPrice(false, $smarty.const.NULL)}</span>&nbsp;{l s='tax excl.'}</p>
...
<!-- minimal quantity wanted -->
					...
					<p id="minimal_quantity_wanted_p"{if $product->minimal_quantity <= 1 OR !$product->available_for_order OR $PS_CATALOG_MODE} style="display: none;"{/if}>
						...
						<span>{l s='This product is not sold individually. You must select at least'} <b id="minimal_quantity_label">{$product->minimal_quantity}</b> {l s='quantity for this product.'}</span>
						...
						<fieldset class="attribute_fieldset">
							...
							{elseif ($group.group_type == 'color')}
								...
								<ul id="color_to_pick_list" class="clearfix">
									...
									<li class="bordercolor {if $group.default == $id_attribute}selected{/if}">
			...
			<p id="product_reference" class="bordercolor" {if isset($groups) OR !$product->reference}style="display: none;"{/if}>
        	...
        	{if $product->description_short OR $packItems|@count > 0}
                ...
                <div id="short_description_block" class="bordercolor">
                   ...
                   <!-- CAMELIA <p class="buttons_bottom_block"><a href="javascript:{ldelim}{rdelim}" class="button">{l s='More details'}</a></p>-->			
					 <p><a href="richiesta-preventivo.php?dati={$product->id};{$product->name};{$product->reference}"><img src="img/preventivo-red.png"></a></p>	

                    {/if}

                    {if $packItems|@count > 0}

                    <div class="short_description_pack bgcolor bordercolor">
						...
						{if ($accessory.allow_oosp || $accessory.quantity > 0) AND $accessory.available_for_order AND !isset($restricted_country_mode)}
							...
							<li class="ajax_block_product bordercolor {if $smarty.foreach.accessories_list.first}first_item{elseif $smarty.foreach.accessories_list.last}last_item{else}item{/if} product_accessories_description">
                            	...
                            	<div class="image_block">
                                	...
                                	<a href="{$accessoryLink|escape:'htmlall':'UTF-8'}" title="{$accessory.legend|escape:'htmlall':'UTF-8'}" class="product_image"><img class="bordercolor"src="{$link->getImageLink($accessory.link_rewrite, $accessory.id_image, 'medium_default')}" alt="{$accessory.legend|escape:'htmlall':'UTF-8'}"  /></a>
								...
								<div class="product_desc">
									...
									<h5 class="s_title_block">
                                        ...
                                        <a class="product_lnk" href="{$accessoryLink|escape:'htmlall':'UTF-8'}">{$accessory.name|escape:'htmlall':'UTF-8'|truncate:50:'...'}</a>

                                    </h5>

									<div class="product_dsc">

										{$accessory.description_short|strip_tags|truncate:200:'...'}
								...
								<div class="button_block bordercolor">
