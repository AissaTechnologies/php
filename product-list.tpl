...
<ul id="product_list" class="list">
  {foreach from=$products item=product name=products}
	<li class="ajax_block_product bordercolor product_list-{cycle name='product_list' values='1,2,3'}">
		...
		<div class="center_block bordercolor">
			<div class="product_flags">
				{if isset($product.new) && $product.new == 1}<span class="new bordercolor">{l s='New'}!</span>{/if}
			...
			<h3><a class="product_lnk" href="{$product.link|escape:'htmlall':'UTF-8'}" title="{$product.name|escape:'htmlall':'UTF-8'}">{$product.name|escape:'htmlall':'UTF-8'|truncate:45:'...'}</a></h3><br>
			<!--CAMELIA, SHORT DESC. E IMAGINI NELLA LISTA PRODOTTI <div class="product_dsc">{$product.description_short|strip_tags:'UTF-8'|truncate:320:'...'}</div>-->
			<div class="product_reference">Riferimento: {$product.reference}</div>
			<div class="product_dsc">{$product.description_short}</div>				
		</div>																				 
		<div class="right_block">
        	{if isset($product.on_sale) && $product.on_sale && isset($product.show_price) && $product.show_price && !$PS_CATALOG_MODE}<span class="on_sale discount">{l s='On sale!'}</span>
           ...
           {if isset($comparator_max_item) && $comparator_max_item && !isset($instant_search)}
			...
			<!--<a class="fdetail" href="{$product.link|escape:'htmlall':'UTF-8'}" title="{l s='View'}">{l s='View'}</a><br/>-->
			
			<!-- CAMELIA, RICHIESTA PREVENTIVO -->				
				<a class="fdetail"  style="color: red;" href="richiesta-preventivo.php?dati={$product.id_product};{$product.name};{$product.reference}"><img src="img/preventivo-green.png"></a>
			<!-- FINE CAMELIA, RICHIESTA PREVENTIVO -->
